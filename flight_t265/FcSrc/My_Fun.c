...
void Set_Position(u8 position_num)
{
	...
	float world_vel_z = PID_Cal(&pos_z_pid, position_z_set[position_num], raspi.z);
	
	float vector_world_vx[2] = {world_vel_x, 0}; 
	float vector_flight_vx[2];
	rot_vec_2(vector_world_vx, (float)my_sin(raspi.angle/100.0*ONE_PI/180.0), vector_flight_vx);
	
	float vector_world_vy[2] = {0, world_vel_y};
	float vector_flight_vy[2];
	rot_vec_2(vector_world_vy, (float)my_sin(raspi.angle/100.0*ONE_PI/180.0), vector_flight_vy);
